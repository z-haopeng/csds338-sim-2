public class DropTable {
    ...
    public DropTable(int n) {
        ...
        for(int i = 0; i < n; i++) {
            chopsticks[i] = true;
            philosophers[i] = new Philosopher(i+1, 0.5, 0.3, Math.floor(((i*0.25)/n+0.1)*100)/100);
        }
    }

    public int[][] run(int t) {
        for(int i = 1; i <= t; i++) {
            // System.out.println("Time = " + i + "\tHungry prob: 0.5  Drop prob: 0.3");
    ...
    private class Philosopher {
        ...
        private double hungry, drop, full;

        public Philosopher(int n, double h, double d, double f) {
            number = n;
            hold = new double[2];
            request = new double[2];
            eat = wait = 0;
            hungry = h; drop = d; full = f;
        }
        ...
        public void step() {
            ...
            // Check if dropping
            else if ((hold[0]!=0 || hold[1]!=0) && Math.random() < drop) {
                drop();
            }
        }
